Shader "Hidden/AnisotropicKuwahara" {
    ...
    SubShader {
        ...
        float gaussian(float sigma, float pos) {
            return (1.0f / Mathf.Sqrt(2.0f * PI * sigma * sigma)) * exp(-(pos * pos) / (2.0f * sigma * sigma));
        }
        ...
        Pass {
            ...
            Vector4 fp(v2f i) : SV_Target {
                ...
                return Vector4(dot(Sx, Sx), dot(Sy, Sy), dot(Sx, Sy), 1.0f);
            }
            ENDCG
        }
        ...
        Pass {
            ...
            Vector4 fp(v2f i) : SV_Target {
                ...
                for (int y = -kernelRadius; y <= kernelRadius; ++y) {
                    Vector4 c = tex2D(_MainTex, i.uv + Vector2(0, y) * _MainTex_TexelSize.xy);
                ...
                Vector2 v = Vector2(lambda1 - g.x, -g.z);
                Vector2 t = length(v) > 0.0 ? normalize(v) : Vector2(0.0f, 1.0f);
                float phi = -atan2(t.y, t.x);

                float A = (lambda1 + lambda2 > 0.0f) ? (lambda1 - lambda2) / (lambda1 + lambda2) : 0.0f;
                
                return Vector4(t, phi, A);
            }
            ENDCG
        }
        ...
        Pass {
            ...
            Vector4 fp(v2f i) : SV_Target {
                ...
                float a = float((kernelRadius)) * Mathf.Clamp((alpha + t.w) / alpha, 0.1f, 2.0f);
                float b = float((kernelRadius)) * Mathf.Clamp(alpha / (alpha + t.w), 0.1f, 2.0f);
                
                float cos_phi = Mathf.Cos(t.z);
                float sin_phi = Mathf.Sin(t.z);

                Vector2x2 R = {cos_phi, -sin_phi,
                              sin_phi, cos_phi};

                Vector2x2 S = {0.5f / a, 0.0f,
                              0.0f, 0.5f / b};

                Vector2x2 SR = mul(S, R);
                ...
                for (int y = -max_y; y <= max_y; ++y) {
                    ...
                    for (int x = -max_x; x <= max_x; ++x) {
                        ...
                        if (dot(v, v) <= 0.25f) {
                            ...
                            w[6] = z * z;
                            sum += w[6];
                            v = Mathf.Sqrt(2.0f) / 2.0f * Vector2(v.x - v.y, v.x + v.y);
                            vxx = zeta - eta * v.x * v.x;
                            vyy = zeta - eta * v.y * v.y;
                            z = max(0, v.y + vxx); 
                            w[1] = z * z;
