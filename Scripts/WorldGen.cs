...
public partial class WorldGen : Node3D
{
    ...
    async Task<ChunkRenderData> GetChunkMesh(Chunk chunkData, int id)
    {
        ...
        foreach (Brush currentBrush in chunkData.brushes)
        {
            ...
            for (int i = 0; i < currentBrush.vertices.Count; i += 3)
            {
            ...
            for (int i = 0; i < currentBrush.indicies.Count; i++)
            {
                vert.X = currentBrush.vertices[currentBrush.indicies[i] / 3];
                vert.Y = currentBrush.vertices[(currentBrush.indicies[i] / 3) + 1];
                vert.Z = currentBrush.vertices[(currentBrush.indicies[i] / 3) + 2];
    ...
    [GlobalClass]
    public partial class Brush : GodotObject
    {
        public Array<byte> vertices;
        public Array<byte> indicies;
        public Array<uint> textures;
