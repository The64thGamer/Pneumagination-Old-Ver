...
public partial class WorldGen : Node3D
{
    ...
    async Task<ChunkRenderData> GetChunkMesh(Chunk chunkData, int id)
    {
        ...
        List<Vector3> allNewVerts = new List<Vector3>();
            ...
            if (triangleAdjacencyList.TryGetValue(currentVert, out adjacentTriangleIndices))
            {
                ...
                for (int i = 0; i < adjacentTriangleIndices.Count; i += 3)
                {
                    for (int e = 0; e < 3; e++)
                    {
                        ...
                        {
                            allNewVerts.Add(currentVert);
                            for (int k = 0; k < 3; k++)
                            {
                                if(indices[adjacentTriangleIndices[i + k]] == currentVertIndiciesNumber)
                                {
                                    indices[adjacentTriangleIndices[i + k]] = verts.Count + allNewVerts.Count - 1;
                                }
                            }
                        }
                    }
        ...
        //Add back new vertices
        verts.AddRange(allNewVerts);
